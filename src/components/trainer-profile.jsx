...
import { User, Phone, Calendar, Award, BookOpen, Clock, Edit, Trash2 } from "lucide-react"
...
export default function TrainerProfile() {
  ...
  const handleDelete = async () => {
    if (window.confirm("Sei sicuro di voler eliminare il tuo profilo trainer?")) {
      setIsDeleting(true)
      ...
      <CardContent className="space-y-6">
        ...
        {trainer.birth_date && (
          <div className="flex items-center gap-3">
            ...
            <span>{new Date(trainer.birth_date).toLocaleDateString("it-IT")}</span>
          </div>
        )}
        ...
        {trainer.certifications && (
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground ml-8">{trainer.certifications}</p>
          </div>
        )}
        ...
        <div className="pt-4 border-t text-xs text-muted-foreground">
          ...
          <p>Aggiornato: {new Date(trainer.updated_at).toLocaleString("it-IT")}</p>
        </div>
      </CardContent>
    </Card>)
  );
